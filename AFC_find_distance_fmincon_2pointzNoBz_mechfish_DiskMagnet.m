...
V=(5e-3)^2*pi*5e-3;
m=Br*V/mu0;
load('test167_15to50cm_OnStick_Mdisk.mat');
f=figure;plot(testset(:,4:9));hold on
...
points1  = [2537 4058 6054 7513 8912 10340 11560 13050];%fixedStick
% points1  = [3523 4216 9351 9841 15510 16160];% moving dStick
realdist = [0.15 0.2  0.25 0.3  0.35 0.4   0.45  0.5  ];

for k = 1 : length(points1)
    for i = 1 : 10 
        params1 = testset(points1(k) + i*30,:);
        figure(f);hold on
        plot(points1(k) + i*30,testset(points1(k) + i*30,4),'y*')
        params2 = testset(points1(k) + i*30 + 10,:);
        plot(points1(k) + i*30+ 10,testset(points1(k) + i*30+ 10,4),'y*')
...
function F = solve_B(a,B1x,B1y,B1z,B2x,B2y,B2z,Besum,ang1,ang2,A1x,A1y,A1z,A2x,A2y,A2z,ang3,ang4)
    ...
    eq8 = Bex1*sin(ang1)*sin(-1*ang2)+Bey1*cos(ang2)*sin(-1*ang1)+Bez1*cos(ang1) - Bdd;
    ...
    eq16 = Bex2*sin(ang3)*sin(-1*ang4)+Bey2*cos(ang4)*sin(-1*ang3)+Bez2*cos(ang3) - Bdd;
