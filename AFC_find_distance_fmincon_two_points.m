...
for i = 1 : length (xarray)
    ...
    %% Distance estimation
    a0=[0.1 0.1 0.1 pi/6 pi/6 1 1 1 0.1 0.1 pi/6 1 1 1];
    lb=[0.05 -0.15 -0.1 pi/4 pi/6 -60 -60 -60 -0.15 -0.1 pi/6 -60 -60 -60];
    ub=[0.6 0.15 0.1 pi/1.8 pi/1.8 60 60 60 0.15 0.1 pi/1.8 60 60 60];
...
function F = solve_B(a,B1x,B1y,B1z,B2x,B2y,B2z,Besum,ang1,ang2,A1x,A1y,A1z,A2x,A2y,A2z,ang3,ang4)
    ...
    eq8 = Bex1*sin(ang1)*sin(-1*ang2)+Bey1*cos(ang2)*sin(-1*ang1)+Bez1*cos(ang1);
    
    eq9 = mu0/(4*pi)*(3*x*((m*sin(theta)*cos(phi2))*x+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt(x^2 + y2^2+ z2^2))^5-(m*sin(theta)*cos(phi2))/(sqrt(x^2 + y2^2+ z2^2))^3) * 1e6 + Bex2;
    eq10 = mu0/(4*pi)*(3*y2*((m*sin(theta)*cos(phi2))*x+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt(x^2 + y2^2+ z2^2))^5-(m*sin(theta)*sin(phi2))/(sqrt(x^2 + y2^2+ z2^2))^3) * 1e6 + Bey2;
    eq11 = mu0/(4*pi)*(3*z2*((m*sin(theta)*cos(phi2))*x+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt(x^2 + y2^2+ z2^2))^5-(m*cos(theta))/(sqrt(x^2 + y2^2+ z2^2))^3) * 1e6 + Bez2;
    eq12 = mu0/(4*pi)*(3*(x-0.02)*((m*sin(theta)*cos(phi2))*(x-0.02)+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt((x-0.02)^2 + y2^2+ z2^2))^5-(m*sin(theta)*cos(phi2))/(sqrt((x-0.02)^2 + y2^2+ z2^2))^3) * 1e6 + Bex2;
    eq13 = mu0/(4*pi)*(3*y2*((m*sin(theta)*cos(phi2))*(x-0.02)+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt((x-0.02)^2 + y2^2+ z2^2))^5-(m*sin(theta)*sin(phi2))/(sqrt((x-0.02)^2 + y2^2+ z2^2))^3) * 1e6 + Bey2;
    eq14 = mu0/(4*pi)*(3*z2*((m*sin(theta)*cos(phi2))*(x-0.02)+(m*sin(theta)*sin(phi2))*y2+(m*cos(theta))*z2)/(sqrt((x-0.02)^2 + y2^2+ z2^2))^5-(m*cos(theta))/(sqrt((x-0.02)^2 + y2^2+ z2^2))^3) * 1e6 + Bez2;
    eq15 = sqrt(Bex2^2+Bey2^2+Bez2^2);
    eq16 = Bex2*sin(ang3)*sin(-1*ang4)+Bey2*cos(ang4)*sin(-1*ang3)+Bez2*cos(ang3);
