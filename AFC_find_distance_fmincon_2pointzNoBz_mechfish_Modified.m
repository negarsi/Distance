...
for i = 1 : length(points1)
    ...
    a0=[0.1   0.1   0.1   pi/2     2*pi      1   1   1   0.1   0.1    pi/20   1   1   1   1 ];
    lb=[0.05 -0.15 -0.1   pi/3     1.8*pi   -60 -60 -60 -0.15 -0.1   -pi/4   -60 -60 -60 -60];
    ub=[0.6   0.15  0.1   pi/1.5   2.2*pi    60  60  60  0.15  0.1    pi/4    60  60  60  60];
    
%     if i > 1
% %         a0 = Inet;
% %         lb=[0.05 -0.15 -0.1  Inet(4)-abs(Inet(4)*0.1)  (Inet(5))-pi/6   Inet(6:10)-abs(Inet(6:10))*0.2 (Inet(11))-pi/6 Inet(12:end)-abs(Inet(12:end))*0.2];
% %         ub=[0.6   0.15  0.1  Inet(4)+abs(Inet(4)*0.1)  (Inet(5))+pi/6   Inet(6:10)+abs(Inet(6:10))*0.2 (Inet(11))+pi/6 Inet(12:end)+abs(Inet(12:end))*0.2];
...
figure;plot(results(:,1),'o--'); hold on
plot(realdist,'*-')
plot(results(:,4)/pi,'+--');
plot(results(:,5)/pi,'*--')
