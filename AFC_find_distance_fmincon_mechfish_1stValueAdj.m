...
for i = 1 : length(points)
    ...
    if i==1
        a0=[ 0.05  0.05  0.1    0.1     1   1   1 ];
        lb=[-0.1  -0.1   -pi/2      0      -60 -60 -60];
        ub=[ 0.1   0.1   pi/2     pi      60  60  60];
        ...
        for j = 1 :3
            Inet = fmincon(@(a)solve_B1(a,B1x,B1y,B1z,B2x,B2y,B2z,Besum,ang1,ang2,xarray(1)),a0,[],[],[],[],lb,ub);
            a0 = Inet;
        end
        Inet0=Inet;
    else
        a0=[0.1   0.05  0.05  0.1    0.1     1   1   1 ];
        lb=[0.05 -0.1  -0.1   -pi/2    0      -60 -60 -60];
        ub=[0.6   0.1   0.1   pi/2     pi      60  60  60];
%         a0(4)=Inet0(3);
%         lb(4)=Inet0(3)-pi/8;
%         ub(4)=Inet0(3)+pi/8;
%         lb(5)=Inet0(4)-pi/5;
...
%%  Functions
function F = solve_B1(a,B1x,B1y,B1z,B2x,B2y,B2z,Besum,ang1,ang2,x)
    mu0=4*pi*1E-7;
    Br=1; 
    V=3.9270e-07;
    m=Br*V/mu0;
